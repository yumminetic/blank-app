...
def add_recession_bands_to_fig(fig, recession_data_series): # Expects a Pandas Series for recession data
    ...
    start_date = None
    # Iterate through dates where recession == 1
    # This logic groups consecutive recession months.
    processed_end_dates = set() # To avoid double-drawing bands if data has overlaps

    for date_index, _ in in_recession_periods.items():
        if start_date is None:
            start_date = date_index
        
        # Check if the next day/month is NOT a recession or if it's the end of the series
        ...
        if not is_last_point:
            ...
            if pd.isna(next_date_val) or next_date_val == 0:
                next_is_not_recession = True
        
        if is_last_point or next_is_not_recession:
            end_date = date_index # The current date is the end of this recession segment
            if end_date not in processed_end_dates:
                 # Ensure end_date is after start_date (can happen with single month recessions)
                actual_end_date = end_date + pd.DateOffset(days=1) if end_date == start_date else end_date
                fig.add_vrect(x0=start_date, x1=actual_end_date, 
                              fillcolor="rgba(128,128,128,0.2)", layer="below", line_width=0,
                              name="NBER Recession" if not any(trace.name == "NBER Recession" for trace in fig.data) else None, # Add legend item only once
                              showlegend=not any(trace.name == "NBER Recession" for trace in fig.data)
                             )
                processed_end_dates.add(end_date)
            start_date = None # Reset for the next segment
    # Final check if a recession started and went to the end of the data
...
def create_corr_plot(rolling_corr_data, ticker1, ticker2, window, years=config.YEARS_OF_DATA_CORR):
    ...
    if rolling_corr_data is not None and not rolling_corr_data.empty:
        ...
        fig.update_layout(title=plot_title, annotations=[dict(text=f"No correlation data for {ticker1}/{ticker2} ({window}-day window).", showarrow=False, align='center')])
    else: 
        fig.update_layout(title=plot_title, annotations=[dict(text=f"Could not load correlation data for {ticker1}/{ticker2}.", showarrow=False, align='center')])
    if rolling_corr_data is None or rolling_corr_data.empty: # Ensure title and axes are set even on error
...
def create_iv_skew_plot(calls_df, puts_df, ticker, expiry_date, current_price):
    ...
    if not data_plotted:
        fig.add_annotation(text=f"No valid options IV data for {ticker} expiring {expiry_date} after processing.", showarrow=False, align='center')
        fig.update_layout(yaxis_range=[0, 100]) 
    return fig

def create_fred_plot(series_data, series_id, series_info, recession_data_series=None, show_recession_bands=False): # Expects recession_data_series to be the USREC Series
    fig = go.Figure()
...
def create_fed_jaws_plot(jaws_data_df, recession_data_series=None, show_recession_bands=False): # Expects recession_data_series to be the USREC Series
    ...
    if jaws_data_df is None or jaws_data_df.empty:
        fig.update_layout(title=plot_title, height=500, annotations=[dict(text=f"No data for Fed's Jaws chart.", showarrow=False, align='center')], xaxis_title='Date', yaxis_title='Percent (%)')
        return fig
    ...
    for series_id in config.FED_JAWS_SERIES_IDS:
        if series_id in jaws_data_df.columns: # Check existence in the passed DataFrame
            ...
            print(f"Warning: Series {series_id} not found in Jaws data for plotting.")
...
def create_ffr_pce_comparison_plot(data_df, ffr_series_id, pce_index_series_id, threshold=2.0, recession_data_series=None, show_recession_bands=False): # Expects recession_data_series to be the USREC Series
    ...
    # CRITICAL CHECK: Ensure the necessary series IDs are columns in the DataFrame
    if ffr_series_id not in plot_df.columns:
        fig.update_layout(title=plot_title, annotations=[dict(text=f"Missing FFR data column: '{ffr_series_id}'. Available: {list(plot_df.columns)}", showarrow=False, align='center')]); return fig
    if pce_index_series_id not in plot_df.columns:
        fig.update_layout(title=plot_title, annotations=[dict(text=f"Missing PCE Index data column: '{pce_index_series_id}'. Available: {list(plot_df.columns)}", showarrow=False, align='center')]); return fig

    plot_df.sort_index(inplace=True)
    pce_yoy_col = 'PCE_YoY_Calculated'; plot_df[pce_yoy_col] = plot_df[pce_index_series_id].pct_change(periods=12) * 100
    plot_df.dropna(subset=[ffr_series_id, pce_yoy_col], inplace=True) # Drop based on FFR and *calculated* PCE YoY
    ...
    for _, seg in plot_df.groupby(group_ids):
        ...
        seg_is_risk = (seg[ffr_series_id].iloc[0] - seg[pce_yoy_col].iloc[0]) > threshold # Access checked columns
...
def create_gold_vs_real_yield_plot(data_df, gold_series_id, real_yield_series_id, recession_data_series=None, show_recession_bands=False): # Expects recession_data_series to be the USREC Series
    ...
    plot_df = data_df.copy()
    # CRITICAL CHECK: Ensure the necessary series IDs are columns
    if gold_series_id not in plot_df.columns:
        fig.update_layout(title=plot_title, annotations=[dict(text=f"Missing Gold data column: '{gold_series_id}'. Available: {list(plot_df.columns)}", showarrow=False, align='center')]); return fig
    if real_yield_series_id not in plot_df.columns:
        fig.update_layout(title=plot_title, annotations=[dict(text=f"Missing Real Yield data column: '{real_yield_series_id}'. Available: {list(plot_df.columns)}", showarrow=False, align='center')]); return fig
        
    plot_df.dropna(subset=[gold_series_id, real_yield_series_id], how='any', inplace=True) 
    if plot_df.empty:
