...
def fred_section_ui(section_key_suffix, title, description, series_fetch_logic, plot_function, display_metrics_logic=None, series_select_options=None, default_series_name_key=None, series_id_map=None):
    ...
    with plot_placeholder.container():
        if st.session_state[calculated_key] and st.session_state.get(data_key) is not None:
            fig = plot_function(st.session_state[data_key], st.session_state[show_recession_key], section_key_suffix, series_id_map, default_series_name_key)
...
def plot_single_fred(data_df, show_recession_value, _, __, ___): 
...
def plot_ffr_pce(data_df, show_recession_value, *_): 
    recession_series = data_df.get(config.USREC_SERIES_ID) if data_df is not None and show_recession_value else None
...
def fetch_gold_ry_logic(start_date_key, end_date_key, show_recession_key, data_key, error_key, *_):
    s_ids = [config.GOLD_VS_REAL_YIELD_SERIES_IDS["gold"], config.GOLD_VS_REAL_YIELD_SERIES_IDS["real_yield_10y"]]
    with st.spinner("Fetching Gold & 10Y Real Yield data..."):
        st.session_state[data_key], st.session_state[error_key] = data_fetchers.get_multiple_fred_data(config.fred, s_ids, st.session_state[start_date_key], st.session_state[end_date_key], include_recession_bands=st.session_state[show_recession_key])
def plot_gold_ry(data_df, show_recession_value, *_): 
    recession_series = data_df.get(config.USREC_SERIES_ID) if data_df is not None and show_recession_value else None
    return plotters.create_gold_vs_real_yield_plot(data_df, config.GOLD_VS_REAL_YIELD_SERIES_IDS["gold"], config.GOLD_VS_REAL_YIELD_SERIES_IDS["real_yield_10y"], recession_series, show_recession_value)
def display_gold_ry_metrics(data_df, _): 
    if data_df is not None:
        gold_id = config.GOLD_VS_REAL_YIELD_SERIES_IDS["gold"]; ry_id = config.GOLD_VS_REAL_YIELD_SERIES_IDS["real_yield_10y"]
        latest_g = data_df[gold_id].dropna().iloc[-1] if gold_id in data_df.columns and not data_df[gold_id].dropna().empty else "N/A"
        latest_ry = data_df[ry_id].dropna().iloc[-1] if ry_id in data_df.columns and not data_df[ry_id].dropna().empty else "N/A"
        c1, c2 = st.columns(2)
        c1.metric("Latest Gold Price", f"${latest_g:,.2f}" if isinstance(latest_g, (float,int)) else latest_g)
